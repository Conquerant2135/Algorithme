...
{
    printf(" \n P(x) = ");
    // il faut peut etre rajouter un 3 eme aray en terme de temporaire comme cela pas de warning
    ...
    for (int i = n - 2; i >= 0; i--)
    {
        // il faut regler le probleme de taille ici car ca peut causer probleme
        // si ca seg ici veuillez utiliser l'autre fonction
        if (coeff[i] < 0)
...
void afficher_res(float px, float *coeff, float x, int n)
{
    ...
    printf("\n\n Avec une polynome de horner de la forme : ");
